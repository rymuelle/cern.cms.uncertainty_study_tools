...
coordinates = ['x', 'y', 'z', 'phix', 'phiy', 'phiz']

def make_square(l_list):
    dim = len(l_list)
    list_product = list(product(l_list, repeat=2))
    summed_product = list(map(lambda x: x[0]+x[1], list_product))
    return np.array(summed_product).reshape(dim,dim)

square_coordinates = make_square(coordinates)

def sign_matrix(*args):
    sign_conv_index = list(product([0,1,2,0,1,2], repeat=2))
    signed_list = list(map(lambda x: signConventions[args][x[0]]*signConventions[args][x[1]], sign_conv_index))
    return np.array(signed_list).reshape(6,6)
